...
export async function parse(): Promise<ParsedArgs> {
  let argv = yargs(process.argv.slice(2))
    ...
    .command(
      SERVER_COMMAND,
      "Start in server mode and connect using a web browser",
    ...
    .command(
      ...
      configureReporterCommand,
      () => {},
    )
    .command(
      [COLLECTOR_COMMAND, "$0"],
      "Run collection for the websites",
      configureCollectorCommand,
      () => {},
    )
    .help("help")
    .epilog(
...
function configureCollectorCommand(yargs: Argv) {
  return (
    yargs
      ...
      ) // ask for command and for inspection url
      .alias("m", "max")
      .nargs("m", 1)
      ...
      .check((parsedArgs, _) => {
        ...
        if (
          parsedArgs._[urlPosition] &&
          (parsedArgs._[urlPosition] as string).startsWith("http")
        ) {
