...
export async function collectLinks(page, logger) {
    ...
    const links_all_with_duplicates = await page.evaluate(() => {
        return Array.from(document.querySelectorAll("a[href]")).map((a: HTMLAnchorElement) => {
            return {
...
export async function filterKeywords(links) {
    let rawKeywords = yaml.load(fs.readFileSync(path.join(__dirname, "../assets/keywords.yml"), "utf8")) as string[]
    let keywords = rawKeywords.map(escapeRegExp);
    let keywords_regexp = new RegExp(keywords.join("|"), "i");

    return links.filter((link) => (link.href.match(keywords_regexp) || link.inner_html.match(keywords_regexp)));
}
...
export async function collectCookies(page, start_time) {
    // example from https://stackoverflow.com/a/50290081/1407622
    return (await page._client().send("Network.getAllCookies")).cookies
